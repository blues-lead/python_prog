...
class Car:
    ...
    def fill(self, bensa_maara):
        if bensa_maara < 0:
            print("You cannot remove gas from the tank")
            return
        elif bensa_maara + self.__gas > self.__tank_size:
            self.__gas = self.__tank_size
            #self.printInformation()
            return
        self.__gas += bensa_maara
    ...
    def drive(self, km):
        if km < 0:
            ...
            return
        fuel_needed = (self.__gas_consumption/100)*km
        if fuel_needed >= self.__gas:
            s_km = (self.__gas_consumption)*self.__gas
            self.__odometer += s_km
            self.__gas = 0
        else:
            self.__odometer += km
            self.__gas -= fuel_needed
